...
from isolation.checker import _map_custom, check, exact, floats, tokens
from isolation.isolate import RawRun
from app.models import Problem, Test


def _problem(kind: str = "exact") -> Problem:
    return Problem(
        external_id="c1",
        title="t",
        statement="s",
...
class CheckerTests(unittest.TestCase):
    ...
    def test_check_skips_when_program_failed(self) -> None:
        result = check(_problem(), _test(), "TLE", "")
        self.assertEqual(result.verdict, "TLE")

    def test_check_dispatches_exact(self) -> None:
        self.assertEqual(check(_problem("exact"), _test("3\n"), "OK", "3").verdict, "OK")
