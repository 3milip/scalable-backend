...
from sprawdzarka.isolate import RawRun, program_stderr, run_raw
...
def custom(checker_code: str, inp: str, got: str, expected: str) -> CheckResult:
    if not (checker_code or "").strip():
        return CheckResult("SI", "brak checker_code")
    work = tempfile.TemporaryDirectory()
    data = tempfile.TemporaryDirectory()
    try:
        checker = Path(work.name) / "checker.py"
        checker.write_text(checker_code, encoding="utf-8")
        checker.chmod(0o644)
...
def check(
    ...
    kind = (checker or "exact").lower()
    if kind == "tokens":
        return tokens(stdout, expected)
    if kind in ("float", "floats"):
        return floats(stdout, expected)
    if kind == "custom":
        return custom(checker_code, inp, stdout, expected)
    return exact(stdout, expected)
